...
async function getEntitiesByIdsByField(entityName, argsWhereByField) {
  ...
  const entitiesToReturn = entityIds.map(entityId => entities.find(entity => entity[`${key}Id`] === entityId));
